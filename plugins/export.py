...
class ExportPlugin(BaseAdminPlugin):
    ...
    def _get_objects(self, context):
        ...
        return [dict([
            (force_bytes(headers[i].text), self._format_value(o)) for i, o in
    ...
    def _get_datas(self, context):
        ...
        new_rows.insert(0, [force_bytes(c.text) for c in context['result_headers'].cells if c.export])
        return new_rows
    ...
    def get_xlsx_export(self, context):
        ...
        sheet = book.add_worksheet(
            u"%s %s" % (_(u'Sheet'), force_bytes(model_name)))
    ...
    def _to_xml(self, xml, data):
        if isinstance(data, (list, tuple)):
            ...
            xml.characters(smart_bytes(data))
