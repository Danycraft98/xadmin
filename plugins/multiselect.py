...
class SelectMultipleTransfer(forms.SelectMultiple):
    ...
    def render_opt(self, selected_choices, option_value, option_label):
        ...
        return u'<option value="%s">%s</option>' % (
            escape(option_value), conditional_escape(force_bytes(option_label))), bool(option_value in selected_choices)
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        selected_choices = set(force_bytes(v) for v in value)
        available_output = []
        chosen_output = []
        ...
        for option_value, option_label in chain(self.choices, choices):
            if isinstance(option_label, (list, tuple)):
                available_output.append(u'<optgroup label="%s">' %
                                        escape(force_bytes(option_value)))
