...
class CreateAdminView(ModelFormAdminView):
    ...
    def get_context(self):
        new_context = {
            'title': _('Add %s') % force_bytes(self.opts.verbose_name),
        }
    ...
    def get_breadcrumb(self):
        ...
        item = {'title': _('Add %s') % force_bytes(self.opts.verbose_name)}
    ...
    def post_response(self):
        ...
        msg = _(
            'The %(name)s "%(obj)s" was added successfully.') % {'name': force_bytes(self.opts.verbose_name),
                                                                 'obj': "<a class='alert-link' href='%s'>%s</a>" % (self.model_admin_url('change', self.new_obj._get_pk_val()), force_bytes(self.new_obj))}
        ...
        if "_addanother" in request.POST:
            self.message_user(msg + ' ' + (_("You may add another %s below.") % force_bytes(self.opts.verbose_name)), 'success')
            return request.path
        else:
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def init_request(self, object_id, *args, **kwargs):
        ...
        if self.org_obj is None:
            raise Http404(_('%(name)s object with primary key %(key)r does not exist.') %
                          {'name': force_bytes(self.opts.verbose_name), 'key': escape(object_id)})
    ...
    def get_context(self):
        new_context = {
            'title': _('Change %s') % force_bytes(self.org_obj),
    ...
    def get_breadcrumb(self):
        ...
        item = {'title': force_bytes(self.org_obj)}
    ...
    def post_response(self):
        ...
        msg = _('The %(name)s "%(obj)s" was changed successfully.') % {'name':
                                                                       force_bytes(verbose_name), 'obj': force_bytes(obj)}
        if "_continue" in request.POST:
            self.message_user(
                msg + ' ' + _("You may edit it again below."), 'success')
            return request.path
            ...
            self.message_user(msg + ' ' + (_("You may add another %s below.")
                              % force_bytes(verbose_name)), 'success')
