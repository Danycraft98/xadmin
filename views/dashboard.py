...
class WidgetTypeSelect(forms.Widget):
    ...
    def render(self, name, value, attrs=None):
        ...
        options = self.render_options(force_bytes(value), final_attrs['id'])
        if options:
            output.append(options)
        output.append(u'</ul>')
        output.append('<input type="hidden" id="%s_input" name="%s" value="%s"/>' %
                     (final_attrs['id'], name, force_bytes(value)))
        return mark_safe(u'\n'.join(output))
...
class ModelDashboard(Dashboard, ModelAdminView):
    ...
    @filter_hook
    def get_title(self):
        return self.title % force_bytes(self.obj)
