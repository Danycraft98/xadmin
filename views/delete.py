...
class DeleteAdminView(ModelAdminView):
    ...
    def init_request(self, object_id, *args, **kwargs):
        ...
        if self.obj is None:
            raise Http404(_('%(name)s object with primary key %(key)r does not exist.') % {'name': force_bytes(self.opts.verbose_name), 'key': escape(object_id)})
    ...
    def get_context(self):
        if self.perms_needed or self.protected:
            title = _("Cannot delete %(name)s") % {"name":
                                                   force_bytes(self.opts.verbose_name)}
        else:
    ...
    def post_response(self):
        ...
        self.message_user(_('The %(name)s "%(obj)s" was deleted successfully.') %
                          {'name': force_bytes(self.opts.verbose_name), 'obj': force_bytes(self.obj)}, 'success')
